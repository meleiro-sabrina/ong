...
export default function ClassStudentsPage() {
  ...
  const loadRoster = async (): Promise<any[]> => {
    if (!classId) return [];
    try {
  ...
  const loadAvailableStudents = async (latestRoster?: any[]) => {
    try {
      const res = await fetch(`/api/students-available?classId=${encodeURIComponent(classId)}`, { cache: 'no-store' });
      if (!res.ok) throw new Error('Falha ao carregar alunos');
      const json = await res.json();
      const allStudents = Array.isArray(json.students) ? json.students : [];
      
      // Defensive extra filtering in case roster is fresher than the query result
      const rosterToUse = Array.isArray(latestRoster) ? latestRoster : enrolledStudents;
      const enrolledIds = new Set(rosterToUse.filter((s: any) => s.status === 'active').map((s: any) => s.id));
  ...
  const handleAddStudent = async (studentId: string) => {
    try {
      ...
      if (!res.ok) {
        ...
        let msg = 'Falha ao vincular aluno';
        try {
      ...
      toast.error('Não foi possível vincular o aluno', typeof e?.message === 'string' ? e.message : 'Erro ao vincular aluno');
    }
  };

  const handleRemoveStudent = async (enrollmentId: string) => {
    confirmDelete('aluno da turma', async () => {
      try {
        ...
        const res = await fetch(`/api/enrollments?id=${encodeURIComponent(enrollmentId)}`, {
          method: 'DELETE',
        });
        if (!res.ok) {
          const txt = await res.text().catch(() => '');
          let msg = 'Falha ao remover aluno';
          try {
        ...
        toast.success('Aluno removido', 'O aluno foi removido da turma com sucesso.');
        await loadRoster();
        await loadAvailableStudents();
      } catch (e: any) {
        toast.error('Não foi possível vincular o aluno', typeof e?.message === 'string' ? e.message : 'Erro ao vincular aluno');
      ...
      message: 'Tem certeza que deseja remover este aluno da turma? Esta ação pode ser revertida adicionando o aluno novamente.',
      confirmText: 'Sim, remover'
    });
  };

  const openAddModal = async () => {
    const latest = await loadRoster();
    await loadAvailableStudents(latest);
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      toast.error('Não foi possível salvar a chamada', typeof e?.message === 'string' ? e.message : 'Erro ao salvar chamada');
    } finally {
