...
export default function AttendancePage() {
  ...
  const handleBuscarLista = async () => {
    if (!selectedClass) {
      toast.warning('Selecione uma turma', 'Por favor, selecione uma turma antes de buscar a lista.');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      if (existingAttendance) {
        toast.info('Lista carregada', className + ' - ' + activeEnrollments.length + ' alunos encontrados. Presença já registrada.');
      } else {
        toast.info('Lista carregada', className + ' - ' + activeEnrollments.length + ' alunos encontrados. Pronto para registrar presença.');
      }
    } catch (e: any) {
      toast.error('Erro ao buscar lista', typeof e?.message === 'string' ? e.message : 'Tente novamente mais tarde');
  ...
  const saveJustification = () => {
    if (justificationModal.studentId) {
      const studentName = students.find(s => s.id === justificationModal.studentId)?.name;
      setStudents(students.map(s => 
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      toast.error('Erro ao salvar presenças', typeof e?.message === 'string' ? e.message : 'Tente novamente mais tarde');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <MobileSelect
              ...
              placeholder="Selecione uma turma ativa..."
              className="w-full"
            />
          </div>
          <div>
