...
export default function StudentsPage() {
  ...
  const { data: studentsData, loading, error: studentsError, refetch } = useStudents(1, 10);
  ...
  const {
    ...
    goToNextPage,
    goToPrevPage,
  } = usePagination({ items: filteredStudents, itemsPerPage: 10 });
    ...
    <div className="max-w-6xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <tr key={student.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => handleDelete(student.id)} className="p-1.5 text-slate-400 hover:text-ngo-danger rounded-md hover:bg-red-50 transition-colors" title="Excluir">
          ...
          {paginatedItems.map((student: any) => (
            <div key={student.id} className="p-4 space-y-3">
              ...
              <div className="flex justify-end space-x-2 pt-2 border-t border-slate-100">
                ...
                <button onClick={() => handleDelete(student.id)} className="p-2 text-slate-600 hover:text-ngo-danger rounded-md hover:bg-red-50 transition-colors flex items-center text-xs font-medium">
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between text-sm text-slate-500 gap-4">
          <div>Mostrando {startItem} a {endItem} de {filteredStudents.length} alunos</div>
